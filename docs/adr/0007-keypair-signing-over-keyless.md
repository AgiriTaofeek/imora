...
- **Keyless signing (the modern default):** rejected specifically for air-gapped verification — silently breaks the "no external system in the required path" rule from system-context.md if adopted without modification.
...
- Key-pair rotation for the signing key itself follows the same versioned-key principle as the vault's KEK rotation (ADR-adjacent to encryption.md), not a separate scheme invented for this purpose.
