...
`browser-sdk`, `ingestion`, and `query-api` share exact entity definitions from domain-model.md (a deliberate Shared Kernel relationship per bounded-contexts.md). Checking how the closest comparator (PostHog) organizes its own monorepo surfaced a warning, not just a precedent: PostHog-FOSS is a separate mirror with "all proprietary code removed" — meaning PostHog's monorepo itself contains the `ee/`-directory license-gating pattern ADR 0001 already rejected.
