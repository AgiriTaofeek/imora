...
- **Source-available (Sentry's FSL, HashiCorp's old BSL):** rejected — not OSI-approved, and procurement teams at regulated orgs actively screen license type as an evaluation criterion, per target-users.md.
...
- Revenue has to come from Milestone 3 (managed hosting, support SLAs, SSO, multi-region tooling) per pricing.md — never from gating core functionality.
