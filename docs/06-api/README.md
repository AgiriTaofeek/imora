...
`docs/06-api/README.md#webhooks` should specify the asynchronous/push side this document deliberately left out. `docs/06-api/openapi.yaml` should formalize the resource surface above into an actual spec.
...
`docs/06-api/README.md#rest-api` should specify the backend surface this SDK talks to (via `gateway`) and the separate programmatic API for DSAR-style queries. `docs/09-workflows/README.md#sdk-installation` should walk through this API from a first-time integrator's perspective.
...
`docs/06-api/openapi.yaml` is the last file in `06-api/` — it should formalize [REST API](README.md#rest-api)'s resource surface and this document's payload shapes into an actual spec.
