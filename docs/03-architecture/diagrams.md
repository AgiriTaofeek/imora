...
| **dashboard** | Static SPA | Presentation only — zero domain entities, zero audit-log authority, per [Bounded Contexts](../02-domain/README.md#bounded-contexts) | Calls `query-api` exclusively |
...
- Internal structure of `browser-sdk`, `gateway`, `ingestion`, `alert-engine`, `notification-service`, `dashboard` — each gets this level of detail in its own `docs/04-services/*.md` if warranted, not here.
