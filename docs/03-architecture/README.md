...
`docs/03-architecture/README.md#scaling` should define the concrete threshold at which the single-machine profile stops being viable and a cluster migration is warranted. `docs/12-infrastructure/README.md#docker-compose` and `README.md#kubernetes` can now be written directly against the two profiles here.
...
├── dashboard/                  # frontend SPA, consumes query-api only per README.md#bounded-contexts's
