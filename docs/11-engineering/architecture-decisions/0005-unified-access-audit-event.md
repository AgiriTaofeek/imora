...
> Status: Accepted. Full reasoning in [Domain Model](../../02-domain/README.md#domain-model), [Event Catalog](../../02-domain/README.md#event-catalog), and [Audit Logging](../../07-security/README.md#audit-logging).
...
- `access_audit_events`' own retention clock must be at least as long as the longest clock among everything it audits — an audit record about a since-deleted SessionEvent still has to prove the deletion was lawful (see [ClickHouse Schema](../../05-data/README.md#clickhouse-schema)).
- Adding `CONFIG_CHANGED` after the fact required touching three already-written files (README.md#event-catalog, README.md#event-schema, README.md#clickhouse-schema) to stay consistent — a real cost of the unified-entity choice, paid once, rather than an ongoing cost of a fragmented one.
