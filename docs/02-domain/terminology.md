...
> Status: Consolidates terms already precisely defined across `00-overview/`, `01-product/`, and this folder into one reference. Every definition here traces back to a specific prior document — this file doesn't introduce new meaning, it collects it.

---

## Positioning Terms

**Parity** — a capability necessary to be a credible alternative to Sentry, Datadog RUM, LogRocket, FullStory, OpenReplay, or PostHog at all. Defined in [vision.md](../00-overview/vision.md)'s Positioning section; the full checklist is in [competitive-analysis.md](../00-overview/competitive-analysis.md).

**Wedge** — a capability none of those alternatives, self-hosted or SaaS, currently ship. The three uncontested wedge gaps (access-audit-trail, regulatory-clock retention, evidence export) are ranked in [competitive-analysis.md](../00-overview/competitive-analysis.md)'s Synthesis.

**Alternative** — Imora's own positioning, per [vision.md](../00-overview/vision.md): not a new category, a swap-in replacement for tools regulated teams already use, plus the wedge.
...
Full definitions in [target-users.md](../00-overview/target-users.md) (roles) and [user-personas.md](../01-product/user-personas.md) (grounded scenarios).

---

## Domain Entities (full definitions in [domain-model.md](domain-model.md))
...
- **AccessAuditEvent** — the append-only log entry produced for every VIEW, EXPORT, UNMASK, or DELETE against sensitive data. See [event-catalog.md](event-catalog.md) for the full set of named variants (SessionViewed, FieldUnmasked, RecordExported, RecordDeleted, DeletionSkippedDueToHold).
...
## Business Rule Shorthand (full rules in [business-rules.md](business-rules.md))
...
- **BAA** — Business Associate Agreement; the HIPAA-required contract that makes a vendor's handling of PHI compliant — a contractual promise about a third party's environment, not organizational control, per [competitive-analysis.md](../00-overview/competitive-analysis.md).
- **CIPA** — California Invasion of Privacy Act; the wiretapping statute (§631, plus §638.51 pen-register claims) behind the session-replay litigation wave in [problem-statement.md](../00-overview/problem-statement.md).
...
## Architecture Terms (full definitions in [bounded-contexts.md](bounded-contexts.md))
...
- **Shared Kernel** — two contexts operating directly on the same entity definitions from [domain-model.md](domain-model.md) (browser-sdk↔ingestion, ingestion↔query-api).
...
This is the last file in `docs/02-domain/`. All five files — [domain-model.md](domain-model.md), [bounded-contexts.md](bounded-contexts.md), [business-rules.md](business-rules.md), [event-catalog.md](event-catalog.md), and this one — are now internally consistent and cross-referenced. `docs/04-architecture/` is the natural next tier: it turns [bounded-contexts.md](bounded-contexts.md)'s context map into actual system-context, container, and component diagrams.
