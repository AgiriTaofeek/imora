...
> Status: Accepted. Full reasoning in [container-diagrams.md](../../04-architecture/container-diagrams.md), [clickhouse-schema.md](../../06-data/clickhouse-schema.md), and [postgres-schema.md](../../06-data/postgres-schema.md).
...
- Cross-store consistency is an application-level responsibility, explicitly documented rather than assumed — see [postgres-schema.md](../../06-data/postgres-schema.md)'s "Common Design Decisions" section.
...
- The single-machine deployment profile ([deployment-model.md](../../04-architecture/deployment-model.md)) has to run both stores on one host, which is the dominant resource-sizing driver for that profile (ClickHouse at 4-core/16GB is the floor).
