...
> Status: Living index — add one file per decision, numbered sequentially, following the Context / Decision / Alternatives Considered / Consequences shape used below. Each ADR distills reasoning that lives in full elsewhere in `docs/`; the ADR is a searchable decision log, not a duplicate of that reasoning.
