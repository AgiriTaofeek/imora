...
- **Licensing model is undecided** and directly affects whether "self-hosted" actually satisfies Dara's and Adaeze's requirement in [user-personas.md](user-personas.md) — a source-available license with SaaS-only premium features (the Sentry model) would undermine the parity claim that a regulated org can run the full stack, control plane included. This belongs in `docs/01-product/licensing.md`, flagged here because it's a prerequisite the PRD depends on, not a detail to solve after the fact.
