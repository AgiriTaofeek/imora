...
This `research/` tree is a general-purpose framework for planning and documenting a software project end to end — from why it should exist through how it runs in production. It was built out for **Imora** (a self-hosted frontend observability platform), so the content inside each file is Imora-specific, but the 13-folder structure itself is meant to be reused as-is for other projects.
