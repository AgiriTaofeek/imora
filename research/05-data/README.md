...
| `environment` | string | Yes | Free-text tag (`production`, `staging`, `development`, or a team's own values) set at SDK `init()` time, per [Domain Model](../02-domain/README.md#domain-model)'s Environment note. Denormalized onto every event for the same reason `releaseId` is — no join in the column store. |
...
| `subjectId` | UUID | Yes | |
| `environment` | string | Yes | Per [Event Catalog](../02-domain/README.md#event-catalog) — baseline and new value are always compared within one environment. |
...
|---|---|---|---|
| `releaseId` | string | Yes | |
| `environment` | string | Yes | Part of the identity of a "release" — the same `releaseId` deployed to two environments produces two `ReleaseDeployed` events. |
| `priorReleaseId` | string | No | Null for the first release **in this environment** — not null just because the same version already exists in a different one. |
...
| `release_id` | String | |
| `environment` | LowCardinality(String) | Per [Domain Model](../02-domain/README.md#domain-model)'s Environment note — low-cardinality-typed since the realistic value set (`production`/`staging`/`development`, or a handful of team-defined names) is exactly what that ClickHouse type optimizes for. |
| **ORDER BY** | `(session_id, occurred_at)` | Replay reconstruction is always "give me this session's events in order" — the dominant query shape. A single session is already scoped to one environment by construction, so `environment` doesn't need to lead here. |
...
| `environment` | LowCardinality(String) | |
| `occurred_at` | DateTime64 | |
| **ORDER BY** | `(environment, release_id, metric_type, occurred_at)` | `environment` leads here — unlike the other two tables, regression detection's dominant query is "compare releases within one environment" ([Event Catalog](../02-domain/README.md#event-catalog)'s `RegressionDetected`), which is a table scan across releases *within* an environment, not within a single session. |
...
Sources: [Manage data with TTL — ClickHouse Docs](https://clickhouse.com/research/guides/developer/ttl), [Support of WHERE and GROUP BY in TTL expressions — ClickHouse GitHub PR #10537](https://github.com/ClickHouse/ClickHouse/pull/10537), [Lightweight delete — ClickHouse Docs](https://clickhouse.com/research/guides/developer/lightweight-delete), [What Is the Difference Between Mutations and Lightweight Deletes in ClickHouse](https://oneuptime.com/blog/post/2026-03-31-clickhouse-mutations-vs-lightweight-deletes/view).

### What This Feeds Next

`research/05-data/README.md#postgres-schema` covers the relational side. `research/05-data/README.md#retention` should turn BR-1's per-category regulatory clocks into the actual `INTERVAL` values used in the TTL clauses above.
...
| `environment` | String | Per [Domain Model](../02-domain/README.md#domain-model)'s Environment note. |
| `release_id` | FK → releases.(id, environment) | Composite, not a bare `releases.id` reference — see below. |
| `device_metadata` | JSONB | |

#### releases
| Column | Type | Notes |
|---|---|---|
| `id` | String | A version identifier (e.g. a git SHA) — **not unique alone.** The same value legitimately appears once per environment it's deployed to. |
| `environment` | String | |
| **PK** | `(id, environment)` | Composite, per [Domain Model](../02-domain/README.md#domain-model)'s Environment note — `id` and `environment` together identify one release row, since the same version reaching `staging` then `production` is two separate deployments, two separate rows. |
| `deployed_at` | Timestamp | |
| `prior_release_id` | FK → releases.(id, environment), nullable | Regression baseline comparison, story C1 — the FK is composite so it always resolves to the prior release **in this same row's environment**, never a same-named release deployed elsewhere. |
...
`research/05-data/README.md#retention` should formalize the `retention_period`/`regulatory_basis` values in `retention_policies` against BR-1's actual regulatory clocks. `research/05-data/README.md#storage` should specify the object-storage (MinIO) layout that `evidence_exports.object_storage_path` points into.
...
`research/05-data/README.md#storage` is the last file in `05-data/` — it should specify the object-storage layout for EvidenceExport blobs, and note how tiered storage (mentioned above but not specified) would interact with the retention periods this document sets.
