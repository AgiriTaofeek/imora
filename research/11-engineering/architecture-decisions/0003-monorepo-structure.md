...
> Status: Accepted. Full reasoning in [Repository Structure](../../03-architecture/README.md#repository-structure).

## Context

`browser-sdk`, `ingestion`, and `query-api` share exact entity definitions from [Domain Model](../../02-domain/README.md#domain-model) (a deliberate Shared Kernel relationship per [Bounded Contexts](../../02-domain/README.md#bounded-contexts)). Checking how the closest comparator (PostHog) organizes its own monorepo surfaced a warning, not just a precedent: PostHog-FOSS is a separate mirror with "all proprietary code removed" — meaning PostHog's monorepo itself contains the `ee/`-directory license-gating pattern ADR 0001 already rejected.
