...
> Status: Accepted. Full reasoning in [Container Diagrams](../../03-architecture/diagrams.md#container-diagrams), [ClickHouse Schema](../../05-data/README.md#clickhouse-schema), and [Postgres Schema](../../05-data/README.md#postgres-schema).
...
- Cross-store consistency is an application-level responsibility, explicitly documented rather than assumed — see [Postgres Schema](../../05-data/README.md#postgres-schema)'s "Common Design Decisions" section.
...
- The single-machine deployment profile ([Deployment Model](../../03-architecture/README.md#deployment-model)) has to run both stores on one host, which is the dominant resource-sizing driver for that profile (ClickHouse at 4-core/16GB is the floor).
